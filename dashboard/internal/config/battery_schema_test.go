...
func TestBatterySchemaCases(t *testing.T) {
	cases := map[string]struct {
		...
		"ac topic in dc only is left to the service": {`[{"id":"b","name":"B","bank_a_voltage_topic":"v","system_type":"dc_only","charger_power_topic":"x"}]`, ""},
		"bank b field in single bank":                {`[{"id":"b","name":"B","bank_a_voltage_topic":"v","bank_b_enabled":false,"bank_b_cell_count":8}]`, ""},
		"inactive field is type-checked": {`[{"id":"b","name":"B","bank_a_voltage_topic":"v","bank_b_enabled":false,"bank_b_cell_count":"x"}]`,
			"$[0].bank_b_cell_count must be integer"},
		"bank a voltage is required": {`[{"id":"b","name":"B","charger_power_topic":"c"}]`,
			"$[0].bank_a_voltage_topic is required"},
		"bank a voltage must not be empty": {`[{"id":"b","name":"B","bank_a_voltage_topic":""}]`,
			"$[0].bank_a_voltage_topic must not be empty"},
		"current unit on ac": {`[{"id":"b","name":"B","bank_a_voltage_topic":"v","charger_power_unit":"A"}]`,
			"$[0].charger_power_unit is not allowed"},
		"unknown unit": {`[{"id":"b","name":"B","bank_a_voltage_topic":"v","charger_dc_power_unit":"kW"}]`,
