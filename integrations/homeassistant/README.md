...
The icons are generated from the Go source in [`../../dashboard/internal/webui/deviceicons.go`](../../dashboard/internal/webui/deviceicons.go) by the tool [`../../scripts/icons/flatten_icons.py`](../../scripts/icons/flatten_icons.py). 

To regenerate the icon set after modifying the source drawings:

```bash
.venv/bin/python scripts/icons/flatten_icons.py
```

This reads `dashboard/internal/webui/icons.source.json` (generated by the Go build), strokes the SVG symbols as filled outlines, and writes them to `custom_components/energy_node_icons/www/energy-node-icons.js`.

The workflow is:
1. Edit the device icon drawings in the Go source (`deviceicons.go`).
2. Build the dashboard to generate `icons.source.json`.
3. Run `flatten_icons.py` to convert them to Home Assistant format.
4. Commit both the updated JS file and the monorepo sources.
